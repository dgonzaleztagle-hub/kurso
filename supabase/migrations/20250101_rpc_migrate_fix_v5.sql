-- ==============================================================================
-- INSTALACIÓN DE LÓGICA DE MIGRACIÓN V5 (Fix Global Constraint)
-- ==============================================================================
-- Este script soluciona el "Duplicate Key" error permitiendo el mismo RUT en diferentes cursos.

-- 1. CORRECCIÓN DE ÍNDICE (La raiz del problema actual)
-- Eliminar la regla de "RUT Único Global" que impide duplicar al alumno para el nuevo año.
DROP INDEX IF EXISTS public.students_rut_unique_idx;

-- Crear la nueva regla: "RUT Único POR CURSO".
-- Esto permite que Juanito tenga el RUT 123 en el curso "5to" y también en el "6to".
CREATE UNIQUE INDEX IF NOT EXISTS students_tenant_rut_idx ON public.students (tenant_id, rut) WHERE (rut IS NOT NULL);

-- 2. RE-INSTALACIÓN DE LA FUNCIÓN (Misma V4, pero para asegurar que está fresca)
...
BEGIN
    ...
    LOOP
        -- A. Crear Estudiante
        -- Usar 'is_active' en lugar de 'status'
